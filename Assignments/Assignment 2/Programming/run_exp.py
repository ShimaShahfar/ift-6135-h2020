...
parser.add_argument('--Q', type=str, default='',
                    help='Question number. If 4.1 or 4.2,load the model.')
...
if args.model == 'RNN':
    model = RNN(emb_size=args.emb_size, hidden_size=args.hidden_size,
                ...
                dp_keep_prob=args.dp_keep_prob, grads={})
elif args.model == 'GRU':
...
###Q4 begin
load_path = None
if (args.Q == '4.1') or (args.Q == '4.2'):
    print('########### Running question %s ############' % args.Q)
    arch = args.model
    load_path = 'best_params/{}_best_params.pt'.format(arch)
...
def run_hgrads(model, data):
    ...
    hidden.retain_grad()
    tt = torch.squeeze(targets.view(-1, model.batch_size * model.seq_len))
    losses = [loss_fn(outputs[t], targets[t]) for t in range(model.seq_len)]
    losses[-1].backward()
    #print(model.grads[f"h{timestep}_{1}"].grad for timestep in range(model.seq_len))
    print("losses:",losses)
    ...
    temp = -10000
    for i in range(35):
        if  torch.norm(torch.mean(model.grads[f"h{i}_{0}"].grad, axis=0)) > temp:
            temp = torch.norm(torch.mean(model.grads[f"h{i}_{0}"].grad, axis=0)) 
    #     temp = torch.mean(model.grads[f"h{i}_{1}"].grad, axis=0)
    #     print(torch.norm(torch.mean(model.grads[f"h{i}_{1}"].grad, axis=0)))
    #     print(np.sqrt(torch.sum(temp**2).cpu()))
    #     print(np.linalg.norm(torch.mean(model.grads[f"h{i}_{1}"].grad, axis=0).cpu()))
    
    
    print("max?", temp)
    hgrads = {'h%d'%(i+1):torch.norm(torch.mean(model.grads[f"h{i}_{0}"].grad, axis=0))/temp for i in range(35)}
    #print("hgrads:")
    #print(model.grads[f"h{1}_{1}"].grad.shape[0])
    #print(hgrads)
    return hgrads
...
if args.Q == '4.1':
    ...
    print(hgrads)
    hgrads = {k:v.cpu() for k,v in hgrads.items()}
    np.save('hgrads/%s_hgrads.npy' % args.model,hgrads)
    print(hgrads)
